<x-pages.datatable :title="t_('code analyse')" :create="false" route="modules.analyse.dashboard.analysis"
                   :datatable="$dataTable">

    <x-slot:prepend>
        <div class="card border-top border-info mb-10 ">
    ...
    <x-slot:actions>
        ...
        <button class='btn btn-sm btn-info' id="analyseBottom" onclick="analyseCode(this)">
            ...
            <span class="indicator-label">{{t_('analyse code')}}</span>
    ...
    <x-slot:scripts>
        <script>
            function analyseCode() {
                var Button = document.querySelector('#analyseBottom');
                $.ajax({
                    url: '{{route('modules.analyse.dashboard.analysis.store')}}',
                    type: "POST",
                    dataType: "JSON",
                    beforeSend: function () {
