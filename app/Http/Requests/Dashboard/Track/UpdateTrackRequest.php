...
class UpdateTrackRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'start_location' => 'string|max:200|different:end_location',
            'start_latitude' => 'string|max:100|different:end_latitude',
            'start_longitude' => 'string|max:100|different:end_longitude',

            'end_location' => 'string|max:200|different:start_location',
            'end_latitude' => 'string|max:100|different:start_latitude',
            'end_longitude' => 'string|max:100|different:start_longitude',
            'end_distance' => 'string|max:100',
            'end_duration' => 'string|max:100',

            "checkPoint_location" => "array|max:10",
            "checkPoint_location.*" => "string|max:200",

            "checkPoint_duration" => "array|max:10",
            "checkPoint_duration.*" => "string|max:100",

            "checkPoint_distance" => "array|max:10",
            "checkPoint_distance.*" => "string|max:100",

            "checkPoint_latitude" => "array|max:10",
            "checkPoint_latitude.*" => "string|max:100",

            "checkPoint_longitude" => "array|max:10",
            "checkPoint_longitude.*" => "string|max:100",
            "map_route_data" => "required",
            "start_time" => "date_format:H:i",
            "repeat" => "array",

            // "total_distance" => "numeric",
            // "total_duration" => "string|max:100",

            "user_vehicle_id" => "exists:user_vehicles,id",
            "driver_id" => "exists:users,id",

            "repeat.*" => "in:Sunday,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday"
        ];
    }
    ...
    public function withValidator(Validator $validator)
    {
        if (request('is_active') == 1) {
            $validator->after(function ($validator) {
                ...
                if (Trip::whereTrackId(request('track')->id)->whereNotNull('start_at')->whereNull('end_at')->count() > 0) {
                    $this->validator->errors()->add(
                        'has ongoing trips',
                        __("messages.the_track_has_ongoing_trips")
                    );
                }

                if (Trip::whereTrackId(request('track')->id)->count() > 0) {
                    $this->validator->errors()->add(
                        'has previous trips',
                        __("messages.the_track_has_previous_trips")
                    );
                }
                ...
                if ($driver) {
                    if (Carbon::now() > $driver->driver_license_end_date) {
                        $this->validator->errors()->add(
                            'license expired',
                            __("messages.the_driver_lisence_is_expired")
                        );
                    }
                }
                ...
                $tracks = auth()
                    ->user()
                    ->tracks()
                    ->whereNotIn("id", [request("track")->id])
                    ->whereIsActive(true)
                    ->where(function ($q) {
                        return $q->whereUserVehicleId($this->user_vehicle_id)
