@if($type == 'App\Notifications\NewContact')
    ...
    @forelse($Notification->take(10) as $notify)
        ...
        <a href="{{route('admin.contact.index').'?id=row_'.get_value('id',$notify->data)}}" class="dropdown-item mark-as-read" data-id="{{ $notify->id }}">
            <div>  <i class="fas fa-joget mr-2"> {{ get_value('name',$notify->data) . " - " . get_value('subject',$notify->data) }} </i> </div>
