@props([ 'name','value'=>data_get(Form::getModel(),$name),'uuid'=>data_get(Form::getModel()?->getFirstMedia($name),'uuid'),'col_size'=>12, 'label'=>null,'info'=>null,'label_class'=>'','multiple'=>null])
@php
...
<div @class(["col-sm-$col_size",'my-6']) >
    <div class="parsley-input @error(dotted_string($name)) parsley-error @enderror mb-1 ">
        ...
        <input type="file" multipl name="{{$name}}"
               @if($value)
